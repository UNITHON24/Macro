...
- 전체 syntax compilation과 표준 라이브러리 테스트를 macOS에서 다시 실행했고, 현재
  suite의 76개 테스트가 통과했다.
- 수정 당시 품질 workflow를 Ubuntu와 Windows에서 통과시켰다. 이후 추가된 acceptance
  gate도 PR #5와 `main`의
  [post-merge quality run](https://github.com/UNITHON24/Macro/actions/runs/29646217944)에서 두
  운영체제 job을 모두 통과해 이식성 회귀가 없음을 확인했다.

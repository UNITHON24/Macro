...
이 폴더는 출처 보존을 위한 레거시 참고 자료입니다. 새 작업과 검증은 저장소 루트 README에서
안내하는 `macro_pkg/` 기준 경로를 사용하세요. 이 스냅샷도 포인터 동작은 기본 dry-run이며,
실제 키오스크가 아닌 격리된 테스트 환경에서만 실행해야 합니다.
...
# 매크로 동작 설정
set KIOSK_DRY_RUN=1  # 기본값. 1: 실제 클릭 안함, 0: 실제 클릭
```
